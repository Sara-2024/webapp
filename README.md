...
**解決策1（推奨）**: 管理者モニターページを常時開く
```
https://webapp-303.pages.dev/admin-monitor
```
- このページを開いている間、**30秒ごと**に自動的にローソク足が生成されます
- リアルタイムチャート表示機能付き（最新100本）
- ページを閉じると生成が停止するため、常時開いておく必要があります

**解決策2**: 外部Cronサービスから定期的に呼び出す
```
https://webapp-303.pages.dev/api/gold10/generate
```
