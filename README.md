...
```
- このページを開いている間、**1秒ごと**に自動的にローソク足が生成されます
- リアルタイムチャート表示機能付き
...
## トレードルール
- **ローソク足**: 1秒足（1秒ごとに確定、リアルタイム更新）
- **ロット**: 1ロット固定
- **最大ポジション数**: 3ポジションまで
- **自動決済**: エントリーから15分経過で自動決済
- **初期残高**: ¥1,000,000
- **価格連続性**: すべてのローソク足は前の足の終値から開始（Gap=0）
...
- **管理者モニター**: https://webapp-303.pages.dev/admin-monitor（1秒ローソク足自動生成）
- **最新の変更**: 
  - ローソク足周期を60秒→1秒に変更
  - 管理者画面にリアルタイムチャート表示機能追加
  - 価格連続性の保証（Open = 前のClose）
