...
**価格変動ロジック（自然な動きに改善）**:
- **変動幅**: ±0.1〜0.5ドル（以前は±0.5〜2.0ドル）
- **トレンド継続**: 70%の確率で前の動きを継承（急激な変動を防止）
- **ヒゲの長さ**: 上ヒゲ0〜0.3ドル、下ヒゲ0〜0.2ドル（以前は0〜1.5ドル）
- **価格連続性**: Open = 前のClose（Gap=0保証）
- **範囲制限**: Close 4900-4945, High 4890-4950, Low 4890-4950
...
- **最新の変更**: 
  - ローソク足生成を管理者モニター画面のみに統一
  - 外部APIエンドポイント（`/api/gold10/generate`）を削除
  - タイムスタンプを毎分00秒と30秒に自動整列
  - チャート表示を最新100本に固定
  - ローソク足の離れ問題を完全解消（open価格補正を削除）
