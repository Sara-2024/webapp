...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>ローソク足自動生成モニター（管理者専用）</title>
    <script src="https://cdn.tailwindcss.com"></script>
    <link href="https://cdn.jsdelivr.net/npm/@fortawesome/fontawesome-free@6.4.0/css/all.min.css" rel="stylesheet">
    <style>
        .log-entry {
    ...
    <div class="container mx-auto p-6 max-w-4xl">
        ...
        <div class="bg-gray-800 rounded-lg shadow-xl p-6 mb-6">
            ...
            <p class="text-gray-400 text-sm">
                ...
                このページを開いている間、30秒ごとに自動的にローソク足が生成されます。
                <br>
                <i class="fas fa-exclamation-triangle mr-2 text-yellow-400"></i>
                <strong>重要：このページは常に開いたままにしてください。</strong>
            </p>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            ...
            <div class="bg-gray-800 rounded-lg shadow-xl p-6 text-center">
                <div class="text-sm text-gray-400 mb-2">稼働状態</div>
            ...
            <!-- 次回実行まで -->
            <div class="bg-gray-800 rounded-lg shadow-xl p-6 text-center">
                <div class="text-sm text-gray-400 mb-2">次回実行まで</div>
                <div id="nextExecution" class="text-2xl font-bold text-blue-400">
                    --秒
                </div>
            </div>
            ...
            <!-- 生成成功数 -->
            <div class="bg-gray-800 rounded-lg shadow-xl p-6 text-center">
                <div class="text-sm text-gray-400 mb-2">生成成功数</div>
                <div id="successCount" class="text-2xl font-bold text-green-400">
                    0本
                </div>
            </div>
        </div>
        ...
        <div class="bg-gray-800 rounded-lg shadow-xl p-6 mb-6">
            <h2 class="text-xl font-bold mb-4">
                <i class="fas fa-chart-candlestick mr-2 text-yellow-400"></i>
                最新ローソク足
            </h2>
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                <div>
                    <div class="text-gray-400">時刻</div>
                    <div id="latestTime" class="font-mono font-bold">--</div>
                </div>
                <div>
                    <div class="text-gray-400">価格</div>
                    <div id="latestPrice" class="font-mono font-bold text-green-400">--</div>
                </div>
                <div>
                    <div class="text-gray-400">RSI</div>
                    <div id="latestRSI" class="font-mono font-bold">--</div>
                </div>
                <div>
                    <div class="text-gray-400">総数</div>
                    <div id="totalCandles" class="font-mono font-bold text-blue-400">--</div>
                </div>
            </div>
        </div>
        ...
        <div class="bg-gray-800 rounded-lg shadow-xl p-6">
            ...
            <div id="activityLog" class="space-y-2 max-h-96 overflow-y-auto">
                <div class="text-gray-400 text-sm">ログを監視中...</div>
            </div>
        </div>
    ...
    <script>
        ...
        function addLog(message, type = 'info') {
            ...
            logContainer.innerHTML = logs.map(log => \`
                <div class="log-entry flex items-start gap-3 p-3 bg-gray-900 rounded-lg">
                    <i class="fas \${icons[log.type]} \${colors[log.type]} mt-1"></i>
                    <div class="flex-1">
                        <span class="font-mono text-xs text-gray-500">\${log.timestamp}</span>
                        <div class="\${colors[log.type]}">\${log.message}</div>
                    </div>
                </div>
            \`).join('');
        }
        ...
        function updateCountdown() {
            const now = Math.floor(Date.now() / 1000);
            const remaining = Math.max(0, nextExecutionTime - now);
            document.getElementById('nextExecution').textContent = remaining + '秒';
        }
        ...
        async function updateLatestCandle() {
            try {
                ...
                if (candles.length > 0) {
                    const latest = candles[candles.length - 1];
                    const time = new Date(latest.timestamp * 1000).toLocaleTimeString('ja-JP', { hour12: false });
        ...
        window.addEventListener('beforeunload', (e) => {
            e.preventDefault();
            e.returnValue = 'このページを閉じると自動生成が停止します。本当に閉じますか？';
        });
    </script>
</body>
</html>
